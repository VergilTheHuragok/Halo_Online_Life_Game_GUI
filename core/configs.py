...
def create_config(config):
    ...
    try:
        ...
        print(config, "file is not in default configs")
        raise e
    config_file.close()
...
def change_config(config_name, key, value):
    ...
    string_offset = 0
    if isinstance(config[key], str):  # TODO: Account for other .__str__()
        # variables that must include '
        string_offset = 2
    parts = ['', '']
    parts[0] = config_string[:config_string.find(key) + len(key) + len("': ")]

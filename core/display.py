...
class Text(object):
    ...
    def __init__(self, text, tooltip=None, hide_tooltip=False,
                 ...
                 new_line=False, whole=None, click=False):
    ...
    def update_font(self):
        if isinstance(self.font, type(None)):
            try:
                ...
                print("Could not load font " + self.font_name + ", using default")
    ...
    def copy(self):
        ...
        if not isinstance(self.tooltip, type(None)):
            copied_tooltip = []
            for text in self.tooltip:
                copied_tooltip.append(text.copy())

        return Text(self.text, copied_tooltip, self.hide_tooltip, self.color,
                    self.highlight_color, self.font_name, self.font_size,
                    self.bold, self.italic, self.new_line, self.whole,
                    self.click)
    ...
    def split(self, index):
        ...
        :type index: int
        """
        halves = [Text(self.text[:index], self.tooltip, self.hide_tooltip,
                       self.color, self.highlight_color, self.font_name,
                       self.font_size, self.bold, self.italic, self.new_line,
                       self.whole, self.click),
                  Text(self.text[index:], self.tooltip, self.hide_tooltip,
                       self.color, self.highlight_color, self.font_name,
                       self.font_size, self.bold, self.italic, self.new_line,
                       self.whole, self.click)]
        ...
        for word in words:
            if word != '':
                ...
                if word_index == len(words) - 1:
                    phrase_modifier = ""
                phrases.append(Text(word + phrase_modifier, self.tooltip,
                                    self.hide_tooltip, self.color,
                                    self.highlight_color, self.font_name,
                                    self.font_size, self.bold, self.italic,
                                    self.new_line, self.whole, self.click))
            else:
                if word_index != len(words) - 1:
...
class TextBox(object):
    ...
    def wrap(self):
        ...
        while len(shortened_text_list) > 0:
            ...
            if self.get_line_width(line) < width and (
                ...
                for wrap_char in priority_word_wrap_chars:
                    ...
                    replace_char = "|"
                    if wrap_char == "|":  # Never replace with current char
                        replace_char = "~"

                    if wrap_char in phrase.get_text():
                        # Check each occurrence for distance to width
                        for i in range(0, phrase.get_text().count(wrap_char)):
                            char_index = phrase.get_text() \
                                .replace(wrap_char, replace_char, i) \
                                .find(wrap_char)
                            current_halves = phrase.split(char_index)
                            current_halves[0].append(wrap_char)
                            current_halves[1].text = current_halves[1].text[1:]
    ...
    def justified_formatting(self):
        ...
        for line in self.all_text_lines:
            if self.pure_justified or (line_num < len(self.all_text_lines) - 1
                ...
                num_spaces = 0
                phrase_num = 0
                current_line = self.strip_line(line)
